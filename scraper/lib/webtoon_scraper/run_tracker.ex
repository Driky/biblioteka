defmodule WebtoonScraper.RunTracker do
  ...
  Handles run completion detection when Crawly spider finishes.

  With the current architecture, Crawly only handles discovery (fetching
  webtoon pages and creating Oban jobs). Chapter processing is handled
  entirely by Oban, so runs can be marked complete immediately when the
  spider finishes.
  """
